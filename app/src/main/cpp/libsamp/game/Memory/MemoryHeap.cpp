...
void* CMemoryHeap::Malloc(uint32 size) {
    ...
    while (blockSize > uint32(block->m_nSize)) {
        ...
        if (block == &m_FreeListEnd) {
            Log("CMemoryHeap[%d]: !!! failed to allocate %d bytes !!!", m_nMemId, block->m_nSize);
            return nullptr;
        }
    }
...
void CMemoryHeap::IntegrityCheck() {
    CHook::CallFunction<void>(g_libGTASA + 0x0055A398 + 1, this);
}
...
void CMemoryHeap::_DumpHeapStatus(bool bCommonSizes) {
    Log("MemoryHeap %x STATUS (used %d bytes out of %d bytes total)", m_FirstBlock, m_nMemUsed, GetHeapSize());
    ...
    Log(" Percentage used:    %.1f", 100.f * float(m_nMemUsed) / float(GetHeapSize()));
    Log(" Largest free block: %d bytes", GetLargestFreeBlock());
    Log(" Holes size:         %d bytes", GetSizeOfHoles());
}
