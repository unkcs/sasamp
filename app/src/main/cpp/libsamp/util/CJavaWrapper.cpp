...
Java_com_nvidia_devtech_NvEventQueueActivity_sendRPC(JNIEnv *pEnv, jobject thiz, jint type,
                                                     jbyteArray str, jint action) {
    switch (type) {
        case 1:
            switch (action) {
                case 1:
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onRegisterSkinBackClick(JNIEnv *pEnv, jobject thiz) {
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onRegisterSkinNextClick(JNIEnv *pEnv, jobject thiz) {
    g_pJavaWrapper->RegisterSkinValue++;
    if (g_pJavaWrapper->RegisterSexMale == 1) // man
    {
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onRegisterChooseSkinClick(JNIEnv *pEnv, jobject thiz,
                                                                       jint choosesex) {
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onRegisterClick(JNIEnv *pEnv, jobject thiz,
    ...
    if (pNetGame) {
        pNetGame->SendRegisterPacket((char *) inputPassword, (char *) inputMail, choosesex,
                                     g_pJavaWrapper->RegisterSkinId);
    }

    pGame->ToggleHUDElement(0, false);

    Log("onRegisterPlayClick: inputPassword - %s, inputMail - %s, ChooseSex - %d", inputPassword,
        inputMail, choosesex);
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onLoginClick(JNIEnv *pEnv, jobject thiz,
                                                          jstring password) {
    const char *inputPassword = pEnv->GetStringUTFChars(password, nullptr);
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onChooseSpawnClick(JNIEnv *pEnv, jobject thiz,
                                                                jint spawnid) {
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onSamwillHideGame(JNIEnv *pEnv, jobject thiz,
                                                               jint samwillpacket) {
...
JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onArmyGameClose(JNIEnv *pEnv, jobject thiz) {
    pNetGame->SendCustomPacket(251, 45, 1);
}

JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onFuelStationClick(JNIEnv *pEnv, jobject thiz,
                                                                jint fueltype, jint fuelliters) {
    pNetGame->SendCustomPacketFuelData(251, 39, fueltype, fuelliters);
}

JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onShopStoreClick(JNIEnv *pEnv, jobject thiz,
                                                              jint buttonid) {
    pNetGame->SendCustomPacket(251, 42, buttonid);
}

JNIEXPORT void JNICALL
Java_com_nvidia_devtech_NvEventQueueActivity_onGunShopClick(JNIEnv *pEnv, jobject thiz,
                                                            jint weaponid) {
...
void
CJavaWrapper::ShowFuelStation(int type, int price1, int price2, int price3, int price4, int price5,
                              int maxCount) {
    JNIEnv *env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showFuelStation, type, price1, price2, price3,
                        price4, price5, maxCount);
}
...
void CJavaWrapper::ShowAuthorization(char *nick, int id, bool ip_match, bool toggleAutoLogin,
                                     bool email_acvive) {
    JNIEnv *env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showAuthorization, jnick, id, ip_match,
                        toggleAutoLogin, email_acvive);
    env->DeleteLocalRef(jnick);
}
...
void CJavaWrapper::ShowChooseSpawn(int organization, int station, int exit, int garage, int house) {
    ...
    env->CallVoidMethod(this->activity, this->s_showChooseSpawn, organization, station, exit,
                        garage, house);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv *env, jobject activity) {
    ...
    s_showAuthorization = env->GetMethodID(nvEventClass, "showAuthorization",
                                           "(Ljava/lang/String;IZZZ)V");
    s_hideAuthorization = env->GetMethodID(nvEventClass, "hideAuthorization", "()V");

    s_showRegistration = env->GetMethodID(nvEventClass, "showRegistration",
    ...
    j_updateAutoShop = env->GetMethodID(nvEventClass, "updateAutoShop",
...
void CJavaWrapper::SendBuffer(const char string[]) {
    ...
    //jclass clazz = env->GetObjectClass(jCasino_LuckyWheel);
    jclass nvEventClass = env->GetObjectClass(activity);
    jmethodID CopyTextToBuffer = env->GetMethodID(nvEventClass, "CopyTextToBuffer",
                                                  "(Ljava/lang/String;)V");
